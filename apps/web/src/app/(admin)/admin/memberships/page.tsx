  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">Manage membership plans and subscribers</p>
      </div>
          ...
          {plans.map(({ plan, package: pkg, subscriberCount }) => (
            <Card key={plan.id} className="p-5">
              ...
              <div className="mt-3 flex items-center justify-between border-t border-luxury-border pt-3">
                <span className="text-xs text-gray-400">{pkg.name} • {plan.interval}</span>
                ...
                {activeMemberships.length === 0 ? (
                  ...
                  activeMemberships.map(({ membership: m, user: u, plan }) => (
                    <TableRow key={m.id}>
                      ...
                      <TableCell className="text-sm text-gray-500">{formatDate(m.startDate)}</TableCell>
