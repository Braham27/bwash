    ...
    <div className="space-y-6">
      ...
      <Card className="p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="flex items-start gap-3">
            <User className="h-4 w-4 text-foreground/30 mt-0.5" />
            <div>
              <p className="text-xs text-foreground/40">Customer</p>
          ...
          <div className="flex items-start gap-3">
            <Phone className="h-4 w-4 text-foreground/30 mt-0.5" />
            <div>
          ...
          <div className="flex items-start gap-3">
            <Clock className="h-4 w-4 text-foreground/30 mt-0.5" />
            <div>
              <p className="text-xs text-foreground/40">Scheduled</p>
              <p className="text-sm">{formatDate(b.preferredDate)} at {formatTime(b.preferredTime)}</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <Car className="h-4 w-4 text-foreground/30 mt-0.5" />
            <div>
              <p className="text-xs text-foreground/40">Vehicle</p>
              <p className="text-sm capitalize">{b.vehicleType}</p>
            </div>
          </div>
          <div className="flex items-start gap-3 sm:col-span-2">
            <MapPin className="h-4 w-4 text-foreground/30 mt-0.5" />
            <div>
              <p className="text-xs text-foreground/40">Address</p>
              <p className="text-sm">{b.address}</p>
            </div>
          </div>
          {b.notes && (
            <div className="flex items-start gap-3 sm:col-span-2">
              <FileText className="h-4 w-4 text-foreground/30 mt-0.5" />
              <div>
                <p className="text-xs text-foreground/40">Notes</p>
                <p className="text-sm text-foreground/70">{b.notes}</p>
              </div>
            </div>
          )}
        </div>
      ...
      {b.accessNotes && (
        <Card className="p-6">
          ...
          <p className="text-sm text-foreground/70">{b.accessNotes}</p>
        </Card>
      )}
            ...
            {history.map(({ entry, changedByUser }) => (
              <div key={entry.id} className="flex items-center justify-between border-b border-luxury-border pb-3 last:border-0">
                <div>
                  ...
                  {entry.notes && (
                    <p className="text-xs text-foreground/40 mt-1">{entry.notes}</p>
                  )}
                </div>
