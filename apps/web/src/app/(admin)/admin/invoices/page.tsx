  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-foreground/50">Manage invoices and payments</p>
      </div>

      {/* Revenue Summary */}
      <div className="grid gap-4 sm:grid-cols-2">
        <Card className="p-5">
          <p className="text-xs text-foreground/40 uppercase tracking-wider">Total Collected</p>
          <p className="text-2xl font-bold text-emerald-400 mt-1">{formatCurrency(total)}</p>
        </Card>
        <Card className="p-5">
          <p className="text-xs text-foreground/40 uppercase tracking-wider">Pending</p>
              ...
              {allInvoices.length === 0 ? (
                ...
                allInvoices.map(({ invoice: inv, user: u }) => (
                  <TableRow key={inv.id}>
                    ...
                    <TableCell className="text-sm text-foreground/60">{formatDate(inv.createdAt)}</TableCell>
