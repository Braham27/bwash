    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Manage wash packages and pricing</p>
        </div>
        <ServiceFormClient />
      </div>
        ...
        {allPackages.map((pkg) => (
          <Card key={pkg.id} className="p-6">
            ...
            <div className="grid grid-cols-3 gap-4 mb-4 pb-4 border-b border-luxury-border">
              <div>
                <p className="text-xs text-gray-400 uppercase tracking-wider">Sedan</p>
                <p className="text-xl font-bold text-gold mt-1">{formatCurrency(pkg.sedanPrice)}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 uppercase tracking-wider">SUV</p>
                <p className="text-xl font-bold text-gold mt-1">{formatCurrency(pkg.suvPrice)}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 uppercase tracking-wider">Truck</p>
